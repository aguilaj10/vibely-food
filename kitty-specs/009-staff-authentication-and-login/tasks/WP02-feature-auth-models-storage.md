...
- 2026-03-26T02:20:41Z – claude-sonnet-4-6 – shell_pid=80734 – lane=done – Review passed (2nd pass): clearToken() Unit fix confirmed, all 3 targets compile. 7 subtasks complete — buildkonfig plugin + AUTH_MODE, domain models (AuthToken/Credentials/User correct imports from core:domain), TokenStorage interface (3 ops), EncryptedSharedPreferencesTokenStorage (AES-256-GCM, withContext(Dispatchers.IO)), Pkcs12KeystoreTokenStorage (PKCS12 keystore, withContext(Dispatchers.IO), TODO password), SessionStorageTokenStorage (sessionStorage, no Dispatchers.IO — correct for sync JS). buildscript resolutionStrategy.force workaround for buildkonfig Kotlin version conflict is sound and isolated to feature:auth only.
