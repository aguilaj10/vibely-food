...
class DatabaseFactoryTest {
    ...
    @Test
    fun `withTenantContext sets all three RLS session variables`() =
        runTest {
    ...
    @Test
    fun `withTenantContext does not set store_id when storeId is null`() =
        runTest {
            val orgId = "a1b2c3d4-e5f6-7890-abcd-ef1234567890"
            val userId = "c3d4e5f6-a7b8-9012-cdef-012345678901"
    ...
    @Test
    fun `RLS session variables are absent after withTenantContext returns`() =
        runTest {
            val ctx =
                TenantContext(
    ...
    @Test
    fun `Flyway creates V1 baseline record in flyway_schema_history`() =
        runTest {
